...
1. **Governance of memory — CLOSED by [`ADR-0003`](../../decisions/ADR-0003-memory-governance.md) (2026-08-05).** *(Original question, preserved:)* This document defines *how* memory works structurally. It does not define *what may be remembered, for how long, or under whose consent* — questions that matter especially because Atlas remembers developing athletes, including minors. The architecture here is deliberately built to accommodate restrictive answers (directed forgetting, derived identity, source retention), and that governance decision was owed to Governance rather than the Intelligence Core. **Resolution:** the governance policy now lives in [`memory-governance.md`](../memory-governance.md), adopted as the governing Tier 1 policy by ADR-0003, which formally closes this open question. This document's structural content is unchanged; only this question's status is recorded as closed.
