...
> **Status:** Active register. Four decisions are open.
...
**One of four is High.** DM-016 moves an Aggregate Map boundary. *(DM-020, previously High, is now resolved — see below; DM-014, previously High and safety-relevant, resolved earlier.)*

**DM-020 is resolved** (Product Owner, 2026-08-12) and therefore absent from the table above. It fixes the **standard of verifiability** for an enforcement-boundary claim: such a claim counts as *implemented* only when it is **demonstrated mechanically at the layer where the boundary is enforced** — never by documentary inspection and never through the UI — via a **reproducible** check that exercises legitimate operations (which must succeed) and unauthorised ones (which must fail), yields **explicit PASS/FAIL** results with **auditable evidence**, and leaves **no persistent test data** wherever it can run transactionally. The **harness/tooling is a consequence, not part of the decision** — no language, framework or suite is prescribed. Recorded in its owning section [`sds.md`](../architecture/sds.md) §9.4, which augments the [`development-rules.md`](../constitution/development-rules.md) §7 Definition of Done for enforcement-boundary work. **First concrete application:** Sprint 3 tenant isolation — S3.2 (policies authored) and **S3.8** (the isolation demonstration), which this decision **unblocks** but does **not** itself execute or close. It resolves **none** of DM-016, DM-017A, DM-019 or DM-022, which remain Open.
