...
> **Status:** Under construction. The blueprint is complete where the domain is decided and explicitly blocked where it is not. **Not approved.** It cannot close before F2 is approved and the decisions in Part VIII answered.
...
### `TrainingSession` · `Attendance` ⚠️ **DM-013**

| | |
|---|---|
| **Columns** | `TrainingSession`: club → · **training → ⚠️ DM-013** · occurred at · coach (user) → · conditions note · frozen — `Attendance`: session → · player → · presence |
| **Constraints** | mutable before it occurs, **append-only after**. `frozen` is the boundary |
| **Deletion** | none |

### `Declaration` ⚠️ **DM-014**

| | |
|---|---|
| **Business identity** | subject + kind + date |
| **Columns** | club → · subject (player · group) · **declarer → ⚠️ DM-014** · **declarer authority ⚠️ DM-014** · kind (momentary · ongoing · occasion) · content · declared at · **session → (optional)** · valid from · superseded by → · expired at |
...
| **Constraints** | ⚠️ DM-015 leaves its lifecycle class undecided. **What is decided is ownership: Atlas holds no exercises** |
...
### `LibraryConcept` · `ConceptRelation` ⚠️ **DM-015**
...
| **Constraints** | relation vocabulary is closed at the Core's **eight types**. A relation versions with its source concept. **Refining a concept must not silently invalidate analyses that referenced a prior version** |
...
| `Training` → `TrainingSession` | ⚠️ **DM-013** | Determines where the coach's modification lives |
...
- **A guardian's safety declaration binds training** though a guardian holds no technical authority — so *the right to write a `Declaration` is not the right to design training* ⚠️ **DM-014**.
...
| **Implements later** | SDS §4 in full · SDS §5 conceptually, pending DM-014 · SDS §6 storage rules · SDS §9's isolation requirement |
...
**Implementation risks.** `0001_atlas_core.sql` still exists and is declared superseded by SDS §4.8; its applied state is unverified · nine decisions block parts of the blueprint. **The first structure is no longer blocked** — DM-023 is resolved.
...
|---|---|
| DM-013 | `TrainingSession.training` · whether it is a root |
| DM-014 | `Declaration.declarer` and `.declarer authority` · the write rule for declarations |
| DM-015 | lifecycle class of `LibraryConcept` and `Exercise` |
...
_This document is the physical blueprint of Atlas. It inherits the SDS and may not contradict it. It cannot close before F2 is approved, G1 resolved, and the decisions above answered._
